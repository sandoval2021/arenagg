...
function Avatar({ entry, size }: { entry: RankingEntry; size: 'sm' | 'lg' }) {
  ...
  return (
    <span className={`relative grid shrink-0 place-items-center overflow-hidden border border-slate-200 bg-slate-50 ${box}`}>
      {entry.crest?.logoUrl ? <img loading="lazy" decoding="async" src={entry.crest.logoUrl} alt="" className="h-full w-full object-contain p-1" /> : entry.avatarUrl ? <img loading="lazy" decoding="async" src={entry.avatarUrl} alt="" className="h-full w-full object-cover" /> : <UserRound className="h-5 w-5 text-[#073B8C]" />}

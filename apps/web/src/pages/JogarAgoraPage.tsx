...
export function JogarAgoraPage() {
  ...
  const challenge = useMutation({
    mutationFn: (userId: string) => challengePlayer(userId),
    onSuccess: async () => {
      ...
      <main className="mx-auto max-w-lg px-4 pt-[max(1rem,env(safe-area-inset-top))]">
        <header className="flex items-center justify-between gap-3 py-3">
          <div className="min-w-0">
            ...
            <p className="mt-1 text-xs font-semibold text-slate-500">Ache alguém online para um amistoso. Ranking e MMR não mudam.</p>
          </div>
        ...
        {activeRoom.data?.room && (
          <button type="button" onClick={() => navigate(`/play/rooms/${activeRoom.data!.room!.id}`)} className="mt-3 flex w-full items-center gap-3 rounded-[1.6rem] border border-violet-200 bg-gradient-to-r from-violet-50 to-blue-50 p-4 text-left shadow-sm">
            ...
            <span className="min-w-0 flex-1"><span className="block text-[9px] font-black uppercase tracking-[.17em] text-violet-600">Amistoso em andamento</span><span className="mt-1 block truncate text-sm font-black">Sua Match Room Casual está pronta</span></span>
          ...
          <section key={item.id} className="mt-3 overflow-hidden rounded-[1.8rem] border border-orange-200 bg-gradient-to-br from-orange-50 via-white to-amber-50 p-4 shadow-lg shadow-orange-100/50">
            <div className="flex items-center gap-3">
              ...
              <div className="min-w-0 flex-1">
                <p className="flex items-center gap-1 text-[9px] font-black uppercase tracking-[.16em] text-orange-600"><BellRing className="h-3.5 w-3.5" /> Desafio de amistoso</p>
                <h2 className="mt-1 truncate text-base font-black">{item.challenger.name}</h2>
                <div className="mt-1 flex items-center gap-2"><RankBadge mmr={item.challenger.mmr} compact /><span className="text-[10px] font-bold text-slate-500">{platformLabels[item.challengerPlatform]}</span></div>
              </div>
              <FriendlyPill />
            </div>
        ...
        <section className="mt-5">
          ...
          <div className="mt-3 flex items-center gap-2 rounded-2xl border border-emerald-100 bg-emerald-50 px-3 py-2.5 text-[11px] font-bold text-emerald-800"><ShieldCheck className="h-4 w-4 shrink-0" />Todos os desafios aqui são amistosos. A patente é apenas visual e o MMR nunca é alterado.</div>

          {outgoing && <div className="mt-3 flex items-center gap-3 rounded-2xl border border-amber-200 bg-amber-50 p-3"><span className="grid h-9 w-9 place-items-center rounded-xl bg-white text-amber-600"><Clock3 className="h-4 w-4" /></span><div className="min-w-0 flex-1"><p className="text-[9px] font-black uppercase tracking-wider text-amber-600">Amistoso enviado</p><p className="truncate text-xs font-black text-slate-700">Aguardando {outgoing.challenged.name}</p></div></div>}
          ...
          <div className="mt-4 grid gap-3 sm:grid-cols-2">
            {listedPlayers.map((player) => (
              <article key={player.queueId} className="rounded-[1.7rem] border border-slate-200 bg-white p-3.5 shadow-sm">
                <div className="flex items-center gap-3"><PlayerAvatar url={player.avatarUrl} name={player.displayName ?? player.name} /><div className="min-w-0 flex-1"><h3 className="truncate text-sm font-black">{player.displayName ?? player.name}</h3><div className="mt-1 flex items-center gap-1.5"><RankBadge mmr={player.mmr} compact /><span className="text-[9px] font-bold text-slate-400">{player.mmr}</span></div></div><span className="shrink-0 rounded-full bg-blue-50 px-2 py-1 text-[9px] font-black text-[#073B8C]">{platformLabels[player.platform]}</span></div>
                <button disabled={!queue || queue.status !== 'ACTIVE' || Boolean(outgoing) || challenge.isPending} onClick={() => challenge.mutate(player.userId)} className="mt-3 min-h-11 w-full rounded-xl bg-[#073B8C] px-3 text-xs font-black text-white shadow-sm disabled:bg-slate-200 disabled:text-slate-400"><Swords className="mr-1.5 inline h-4 w-4" />Desafiar para Amistoso</button>
              </article>
            ))}
          </div>
        </section>
...
function PlayerAvatar({ url, name, size = 'md' }: { url: string | null; name: string; size?: 'md' | 'lg' }) {
  ...
  if (url) return <img src={url} alt="" className={`${classes} shrink-0 border border-slate-200 bg-white object-cover`} loading="lazy" referrerPolicy="no-referrer" decoding="async" />;
...
function FriendlyPill() {
  return <span className="shrink-0 rounded-full bg-emerald-100 px-2 py-1 text-[9px] font-black text-emerald-700">AMISTOSO</span>;
}

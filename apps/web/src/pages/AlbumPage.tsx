...
function CardArtwork({ card }: { card: Pick<AlbumCard, 'imageUrl' | 'name'> }) {
  ...
  if (card.imageUrl && !failed) {
    return <img loading="lazy" decoding="async" src={card.imageUrl} alt={card.name} onError={() => setFailed(true)} className="h-full w-full rounded-[1.1rem] object-cover" />;
  }
  return (
        ...
        <div className="mt-5 grid grid-cols-2 gap-3 sm:grid-cols-3">
          {result.cards.map((card, index) => (
            <article key={`${card.id}-${index}`} className={`relative aspect-[3/4] overflow-hidden rounded-[1.5rem] border p-2 shadow-lg ${rarityStyle[card.rarity]}`}>
              ...
              {card.imageUrl ? <img loading="lazy" decoding="async" src={card.imageUrl} alt={card.name} className="h-full w-full rounded-[1.1rem] object-cover" /> : <div className="grid h-full place-items-center rounded-[1.1rem] bg-white/15 p-3 text-center"><p className="text-sm font-black">{card.name}</p></div>}

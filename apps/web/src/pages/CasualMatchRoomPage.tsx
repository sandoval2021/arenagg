  ...
  return (
    <div className="min-h-dvh bg-white pb-28 text-slate-900">
      <main className="mx-auto max-w-lg px-4 pt-[max(1rem,env(safe-area-inset-top))]">
        ...
        {!isClosed && (
          <>
            <section className="mt-5 rounded-[1.8rem] border border-blue-200 bg-white p-4 shadow-sm">
              <p className="text-[10px] font-black uppercase tracking-[.17em] text-[#073B8C]">EA ID / PSN ID / Gamertag</p>
              <h2 className="mt-1 text-lg font-black">Adicionem-se no console</h2>
              <p className="mt-1 text-xs font-semibold text-slate-500">O ID fica disponível apenas nesta Match Room para vocês combinarem o amistoso.</p>
              ...
              <div className="mt-3 rounded-[1.35rem] border border-amber-200 bg-gradient-to-r from-amber-50 to-yellow-50 p-4 shadow-sm">
                <p className="text-[9px] font-black uppercase tracking-[.16em] text-amber-700">ID de {opponent.name}</p>
                {opponent.handle ? <button type="button" onClick={copyOpponentHandle} className="mt-2 flex w-full items-center justify-between gap-3 text-left"><span className="truncate text-lg font-black text-slate-950">{opponent.handle}</span><span className="flex shrink-0 items-center gap-1 rounded-full bg-white px-2.5 py-1 text-[10px] font-black text-amber-700 shadow-sm"><Copy className="h-3.5 w-3.5" />{copied ? 'Copiado' : 'Copiar'}</span></button> : <p className="mt-2 text-xs font-semibold text-slate-500">Aguardando {opponent.name} informar o ID…</p>}
              </div>

...
function messageFor(error: unknown): string {
  ...
  const details = error.details as { message?: string; error?: string; unresolved?: number } | undefined;
  ...
  const messages: Record<string, string> = {
    ...
    KNOCKOUT_MATCH_CANCEL_FORBIDDEN: 'Partidas do mata-mata não podem ser puladas porque quebrariam a progressão da chave.',
...
export function CompetitionHostActionsPanel() {
  ...
  const cancelMatch = useMutation({
    mutationFn: async () => {
      ...
      if (!window.confirm(`Anular ${selected.homeName} x ${selected.awayName}? A partida ficará CANCELED e não contará na classificação.`)) {
        throw new Error('ACTION_CANCELED');
      }
    ...
    <section className="mx-auto mt-4 max-w-5xl px-4 sm:px-6" aria-label="Ações do Host">
      <div className="overflow-hidden rounded-[1.75rem] border border-blue-200 bg-gradient-to-br from-white via-blue-50/60 to-cyan-50/40 shadow-md shadow-blue-100/50">
        ...
        <div className="grid gap-3 p-3 md:grid-cols-3 md:p-5">
          <ActionCard icon={Ban} title="Pular partida" description="Anula um confronto pendente. CANCELED é terminal e vale 0 jogo/0 ponto.">
        ...
        {mutationError && messageFor(mutationError) !== 'ACTION CANCELED' && <p className="mx-3 mb-3 flex gap-2 rounded-xl border border-amber-200 bg-amber-50 px-3 py-2 text-xs font-bold text-amber-800 md:mx-5 md:mb-5"><ShieldAlert className="h-4 w-4 shrink-0" />{messageFor(mutationError)}</p>}
      </div>
    </section>
  );
}

function ActionCard({ icon: Icon, title, description, children }: { icon: typeof Ban; title: string; description: string; children: React.ReactNode }) {

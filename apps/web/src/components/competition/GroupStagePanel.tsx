      ...
      <div className="overflow-hidden rounded-[2rem] border border-blue-200 bg-white shadow-lg shadow-blue-100/40">
        <header className="bg-gradient-to-r from-[#073B8C] via-blue-700 to-cyan-600 p-3 text-white sm:p-5">
          <div className="flex items-start gap-3">
            ...
            <div className="min-w-0 flex-1"><p className="text-[10px] font-black uppercase tracking-[.2em] text-blue-100">Champions Style</p><h2 className="mt-1 text-xl font-black">Tabela de Classificação</h2><p className="mt-1 text-xs font-semibold text-blue-100">Atualização automática a cada 3s · os 2 melhores avançam.</p></div>
                      ...
                      <table className="w-full min-w-[500px] border-collapse text-[10px] text-slate-700">
                        <thead>
                          <tr className="border-b border-slate-100 bg-white text-[8px] font-black uppercase tracking-wide text-slate-400">
                            ...
                            <th className="w-[110px] px-1.5 py-1.5 text-left">Time</th>
                            ...
                            <th className="px-1.5 py-1.5 text-center">SG</th>
                          </tr>
                        </thead>
                        <tbody>
                          {group.standings.map((row, index) => {
                            ...
                            return (
                              <tr key={row.teamId} className={`border-b border-slate-100 last:border-b-0 ${qualified ? 'bg-emerald-50/70' : 'bg-white'}`}>
                                <td className="px-1.5 py-1.5 text-center"><span className={`inline-grid h-6 w-6 place-items-center rounded-lg text-[10px] font-black ${qualified ? 'bg-emerald-600 text-white' : 'bg-slate-100 text-slate-500'}`}>{index + 1}</span></td>
                                ...
                                <td className="px-1.5 py-1.5 text-center text-[11px] font-black text-[#073B8C]">{row.points}</td>
                                <td className="px-1.5 py-1.5 text-center text-[10px] font-bold text-slate-600">{row.played}</td>
                                <td className="px-1.5 py-1.5 text-center text-[10px] font-bold text-emerald-700">{row.wins}</td>
                                <td className="px-1.5 py-1.5 text-center text-[10px] font-bold text-slate-600">{row.draws}</td>
                                <td className="px-1.5 py-1.5 text-center text-[10px] font-bold text-rose-600">{row.losses}</td>
                                <td className={`px-1.5 py-1.5 text-center text-[10px] font-black ${row.goalDifference > 0 ? 'text-emerald-700' : row.goalDifference < 0 ? 'text-rose-600' : 'text-slate-500'}`}>{row.goalDifference > 0 ? '+' : ''}{row.goalDifference}</td>
                              </tr>
                            );
                          })}
                        </tbody>

      ...
      <div className="overflow-hidden rounded-[2rem] border border-blue-200 bg-white shadow-lg shadow-blue-100/40">
        ...
        {data && data.groups.length > 0 && (
          <div className="p-3 sm:p-5">
                ...
                {data.groups.map((group) => (
                  <article key={group.id} className="w-[min(92vw,420px)] shrink-0 snap-center overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm">
                    ...
                    <div
                      ...
                      tabIndex={0}
                    >
                      <table className="w-full min-w-[460px] border-collapse text-[10px] text-slate-700">
                        <thead>
                          <tr className="border-b border-slate-100 bg-white text-[8px] font-black uppercase tracking-wide text-slate-400">
                            ...
                            <th className="w-[104px] px-1 py-2 text-left">Time</th>
                          ...
                          {group.standings.map((row, index) => {
                            ...
                            return (
                              <tr key={row.teamId} className={`border-b border-slate-100 last:border-b-0 ${qualified ? 'bg-emerald-50/70' : 'bg-white'}`}>
                                <td className="px-1 py-2 text-center"><span className={`inline-grid h-6 w-6 place-items-center rounded-lg text-[10px] font-black ${qualified ? 'bg-emerald-600 text-white' : 'bg-slate-100 text-slate-500'}`}>{index + 1}</span></td>
                                <td className="px-1 py-2">
                                  <div className="flex min-w-0 items-center gap-1.5">
                                    ...
                                    <div className="min-w-0"><p className="max-w-[80px] truncate text-[11px] font-black text-slate-900">{row.team.name}</p>{qualified && <p className="max-w-[80px] truncate text-[7px] font-black uppercase text-emerald-600">Classifica</p>}</div>
                                  </div>
                                </td>
                                <td className="px-1 py-2 text-center text-xs font-black text-[#073B8C]">{row.points}</td>
                                <td className="px-1 py-2 text-center text-[10px] font-bold text-slate-600">{row.played}</td>
                                <td className="px-1 py-2 text-center text-[10px] font-bold text-emerald-700">{row.wins}</td>
                                <td className="px-1 py-2 text-center text-[10px] font-bold text-slate-600">{row.draws}</td>
                                <td className="px-1 py-2 text-center text-[10px] font-bold text-rose-600">{row.losses}</td>
                                <td className={`px-1 py-2 text-center text-[10px] font-black ${row.goalDifference > 0 ? 'text-emerald-700' : row.goalDifference < 0 ? 'text-rose-600' : 'text-slate-500'}`}>{row.goalDifference > 0 ? '+' : ''}{row.goalDifference}</td>
                              </tr>
                            );
                          })}
                        </tbody>
            ...
            <div className="mt-3 flex items-center gap-2 rounded-2xl bg-blue-50 px-3 py-2.5 text-[10px] font-bold text-[#073B8C]"><Sparkles className="h-4 w-4 shrink-0" />Tabela ao vivo: atualização automática a cada 3 segundos. PTS, J, V, E, D e SG nunca são ocultados.</div>

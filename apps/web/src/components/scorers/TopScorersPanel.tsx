...
function ScorerRow({ scorer }: { scorer: TopScorer }) {
  ...
  return (
    <article className={`relative flex items-center gap-2.5 px-4 py-3.5 sm:gap-3 sm:px-5 sm:py-4 ${first ? 'bg-gradient-to-r from-amber-50/90 via-white to-yellow-50/70' : 'bg-white'}`}>
      ...
      {scorer.teamLogoUrl ? (
        <img src={scorer.teamLogoUrl} alt="" className="h-10 w-10 shrink-0 rounded-xl border border-slate-200 bg-white object-contain shadow-sm sm:h-11 sm:w-11" loading="lazy" referrerPolicy="no-referrer" decoding="async" />
      ) : (

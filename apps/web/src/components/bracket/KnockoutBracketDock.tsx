...
function TeamRow({
  ...
  return (
    <div className={`flex items-center gap-2.5 transition ${loser ? 'opacity-50' : 'opacity-100'}`}>
      {team?.logoUrl ? (
        <img decoding="async" src={team.logoUrl} alt="" className="h-8 w-8 shrink-0 rounded-lg border border-slate-200 bg-white object-cover" loading="lazy" referrerPolicy="no-referrer" />
      ) : (
...
function CompactTeam({ team, align }: { team: BracketMatch['homeTeam']; align: 'left' | 'right' }) {
  return (
    <div className={`min-w-0 ${align === 'right' ? 'text-right' : 'text-left'}`}>
      {team?.logoUrl ? <img loading="lazy" decoding="async" src={team.logoUrl} alt="" className={`h-9 w-9 rounded-xl border border-slate-200 bg-white object-cover ${align === 'right' ? 'ml-auto' : ''}`} /> : <span className={`grid h-9 w-9 place-items-center rounded-xl border border-slate-200 bg-white text-slate-400 ${align === 'right' ? 'ml-auto' : ''}`}><Shield className="h-4 w-4" /></span>}

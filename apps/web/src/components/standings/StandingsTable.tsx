            ...
            {standings.map((row, index) => {
              ...
              return (
                <tr key={row.teamId} className="border-b border-slate-100 bg-white transition hover:bg-blue-50/30 last:border-b-0">
                  ...
                  <td className="sticky left-10 z-10 bg-white px-2 py-1.5 sm:left-14 sm:px-3 sm:py-2">{userId ? <Link to={`/profile/${encodeURIComponent(userId)}`} className="group flex min-h-10 min-w-0 items-center gap-2 rounded-xl px-0.5 py-0.5 sm:min-h-12 sm:gap-3 sm:px-1 sm:py-1 transition hover:bg-blue-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-200" aria-label={`Abrir perfil de ${row.playerName}`}>{identity}</Link> : <div className="flex min-h-10 min-w-0 items-center gap-2 px-0.5 py-0.5 sm:min-h-12 sm:gap-3 sm:px-1 sm:py-1">{identity}</div>}</td>
                  <td className="px-2 py-2 text-center sm:px-3 sm:py-3 text-base font-black text-[#073B8C]">{row.points}</td><td className="px-2 py-2 text-center sm:px-3 sm:py-3 font-extrabold text-slate-600">{row.played}</td><td className="px-2 py-2 text-center sm:px-3 sm:py-3 font-extrabold text-emerald-700">{row.wins}</td><td className="px-2 py-2 text-center sm:px-3 sm:py-3 font-extrabold text-slate-600">{row.draws}</td><td className="px-2 py-2 text-center sm:px-3 sm:py-3 font-extrabold text-rose-600">{row.losses}</td><td className={`px-3 py-3 text-center font-black ${row.goalDifference > 0 ? 'text-emerald-700' : row.goalDifference < 0 ? 'text-rose-600' : 'text-slate-600'}`}>{row.goalDifference > 0 ? '+' : ''}{row.goalDifference}</td>
                </tr>
              );
            })}
          </tbody>
...
function TeamLogo({ name, logoUrl, podium }: { name: string; logoUrl?: string; podium: boolean }) {
  if (logoUrl) return <img src={logoUrl} alt="" className={`h-8 w-8 shrink-0 rounded-lg sm:h-10 sm:w-10 sm:rounded-xl border bg-white object-cover shadow-sm ${podium ? 'border-blue-200' : 'border-slate-200'}`} loading="lazy" referrerPolicy="no-referrer" />;

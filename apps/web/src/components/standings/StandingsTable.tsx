        ...
        <table className="w-full min-w-[520px] border-collapse text-[10px] text-slate-700 sm:text-xs">
          ...
          <tbody>
            {standings.map((row, index) => {
              ...
              const effectiveLogo = row.user?.avatarUrl ?? row.logoUrl;
              return (
                  ...
                  <td className="px-1.5 py-2">
                    {userId ? (
                      <Link to={`/profile/${encodeURIComponent(userId)}`} className="group flex min-w-0 items-center gap-1.5 rounded-lg" aria-label={`Abrir perfil de ${row.playerName}`}>
                        <div className="relative shrink-0"><TeamLogo name={row.team} logoUrl={effectiveLogo} podium={podium} />{rank && <span className="absolute -bottom-1 -right-1"><RankEmblem rank={rank} compact /></span>}</div>

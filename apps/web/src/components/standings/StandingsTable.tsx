...
export function StandingsTable({ standings, userIdByTeam = {} }: { standings: Standing[]; userIdByTeam?: Record<string, string> }) {
  ...
  return (
    <div className="overflow-hidden rounded-[1.5rem] border border-slate-200 bg-white shadow-md shadow-slate-200/60 ring-1 ring-slate-100 sm:rounded-[2rem]">
            ...
            {standings.map((row, index) => {
              ...
              return (
                <tr key={row.teamId} className="border-b border-slate-100 bg-white transition hover:bg-blue-50/30 last:border-b-0">
                  ...
                  <td className="px-2 py-2 text-center text-xs font-black text-[#073B8C] sm:px-3 sm:py-3 sm:text-base">{row.points}</td><td className="px-2 py-2 text-center font-extrabold text-slate-600 sm:px-3 sm:py-3">{row.played}</td><td className="px-2 py-2 text-center font-extrabold text-emerald-700 sm:px-3 sm:py-3">{row.wins}</td><td className="px-2 py-2 text-center font-extrabold text-slate-600 sm:px-3 sm:py-3">{row.draws}</td><td className="px-2 py-2 text-center font-extrabold text-rose-600 sm:px-3 sm:py-3">{row.losses}</td><td className={`px-2 py-2 text-center font-black sm:px-3 sm:py-3 ${row.goalDifference > 0 ? 'text-emerald-700' : row.goalDifference < 0 ? 'text-rose-600' : 'text-slate-600'}`}>{row.goalDifference > 0 ? '+' : ''}{row.goalDifference}</td>
                </tr>
              );
            })}
          </tbody>
      ...
      <p className="border-t border-slate-200 bg-slate-50 px-3 py-2.5 text-[10px] font-semibold text-slate-500 sm:px-5 sm:py-3">A patente brilhante é baseada no MMR. Toque no jogador para abrir o perfil.</p>
    </div>
  );
}
